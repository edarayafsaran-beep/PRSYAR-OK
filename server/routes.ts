...
export async function registerRoutes(httpServer: Server, app: Express) {
  ...
  app.post(api.auth.login.path, async (req, res) => {
    try {
      ...
      if (!user) user = await db.createUser({ fullName: input.fullName, militaryId: input.militaryId, role: input.militaryId === "ADMIN123" ? "admin" : "officer" });
  ...
  app.post(api.requests.create.path, requireAuth, async (req: any, res) => {
    try { res.status(201).json(await db.createRequest(req.session.userId!, api.requests.create.input.parse(req.body))); }
    catch { res.status(400).json({ message: "Validation error" }); }
  });
  ...
  app.post(api.requests.reply.path, requireAuth, async (req: any, res) => {
    ...
    } catch { res.status(400).json({ message: "Error creating reply" }); }
  });
