...
export class DatabaseStorage {
  ...
  async getRequestsByUser(userId: number) {
    return await db.query.requests.findMany({ where: eq(requests.userId, userId), orderBy: [desc(requests.createdAt)], with: { user:true, attachments:true, replies:{with:{admin:true}} } });
  }

  async getAllRequests() {
    return await db.query.requests.findMany({ orderBy:[desc(requests.createdAt)], with:{ user:true, attachments:true, replies:{with:{admin:true}} } });
  }

  async getRequest(id: number) {
    return await db.query.requests.findFirst({ where: eq(requests.id, id), with:{ user:true, attachments:true, replies:{with:{admin:true}} } });
  }
  ...
  async createRequest(userId:number, input:CreateRequestInput) {
    return await db.transaction(async (tx) => {
      ...
      if(input.attachments?.length) {
        await tx.insert(attachments).values(input.attachments.map(att=>({requestId:newRequest.id,fileName:att.fileName,fileUrl:att.fileUrl,fileType:att.fileType})));
      }
      return newRequest;
    });
  }
